...
export class PostParseDefHinter {
  checkAndGetTypesFrom() {
    return {
      errLinkOptType(typeIt, types) {},
      invalidLastTypesWarn(inputTypes, defLinkTypesList) {},
    };
  }
  ...
  formatAssetLinkHandler() {
    return {
      noneAstUrlWarn() {},
      invalidAssetname(assetname, sourceDirname) {},
    };
  }
}

let hint = new PostParseDefHinter();
const BACK_TO_TOP = '⬆ back to top';

export class PostParse {
  static injectHint(extHint) {
    hint = extHint;
  }
  ...
  #setConf(conf, preConf = {}) {
    ...
    const checkAndGetTypesFrom = (param0) => {
      ...
      if (lastTypes.length === 0) {
        mHint.invalidLastTypesWarn(types, DEF_LINK_TYPE_LIST);
  ...
  #formatAssetLinkHandler(ast) {
    ...
    let lastUrl = ast?.url;
    const mHint = hint.formatAssetLinkHandler();

    if (!lastUrl) {
      mHint.noneAstUrlWarn();
      return;
    }
    ...
    if (lastUrl) {
      ...
      mHint.invalidAssetname(assetname, sourceDirname);
    }
  }
